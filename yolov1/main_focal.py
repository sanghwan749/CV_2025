...
from utils.loss import FocalLoss      # ← 변경
...
def main(args):
    ...
    for epoch in range(epoch_start, num_epochs):
        ...
        for i, (images, target) in progress_bar:
            ...
            optimizer.zero_grad()
            loss = criterion(pred, target[:, :, :, 0].long())   # class index 사용
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            ...
            for i, (images, target) in progress_bar:
                ...
                loss = criterion(prediction, target[:, :, :, 0].long())
...
if __name__ == '__main__':
    ...
    parser.add.add_argument("--lr", type=float, default=0.001)
    parser.add.add.argument("--data_dir", type=str, default='./Dataset')
    parser.add.add.argument("--pre_weights", type=str)
    parser.add.add.argument("--save_dir", type=str, default="./weights")
    parser.add.add.argument("--img_size", type=int, default=448)
