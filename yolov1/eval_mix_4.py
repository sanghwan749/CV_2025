...
from nets.nn import resnet50     # ★ 필수: torchvision resnet50 → YOUR YOLOv1 모델로 변경
from utils.util import predict   # 기존 decode/predict 코드 그대로 사용
        ...
        for class_name in VOC_CLASSES:
            ...
            for d, image_id in enumerate(image_ids):
                ...
                if (image_id, class_name) in self.targets:
                    ...
                    for gt_box in BBGT:
                        x_min = max(gt_box[0], bb[0])
                        y_min = max(gt_box[1], bb[1])
                        x_max = min(gt_box[2], bb[2])
                        y_max = min(gt_box[3], bb[3])
                        ...
                        union = (bb[2] - bb[0] + 1.) * (bb[3] - bb[1] + 1.) + \
                                (gt_box[2] - gt_box[0] + 1.) * (gt_box[3] - gt_box[1] + 1.) - intersection
...
if __name__ == '__main__':
    ...
    for line in lines:
        ...
        for obj in objects:
            c, x1, y1, x2, y2 = map(int, obj.rstrip().split())
            targets[(image_name, VOC_CLASSES[c])].append([x1, y1, x2, y2])
    print('DONE.\n')
    ...
    model.load_state_dict(torch.load('./weights/yolov1_mix_4_final.pth')['state_dict'])
    model.eval()
    ...
    with torch.no_grad():
        for image_name in tqdm(image_list):
            ...
            for (x1, y1), (x2, y2), class_name, _, conf in result:
